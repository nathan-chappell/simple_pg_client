...
const { getFormatParser, generateParser } = require("./message_parser.js");
...
function test_getByte1FormatParser_valid() {
    ...
    return formatParser.parser(readable).then((_value) => {
        if (_value !== formatParser.expected) {
            throw new Error(`expected: ${formatParser.expected}, got: ${_value} | ${typeof formatParser.expected} - ${typeof _value}`);
        }
    });
}
